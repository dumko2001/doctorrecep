...
// Removed 'Tables' from the import as it's not directly used here
import { ApiResponse, DoctorWithStats, AdminDashboardStats, AdminActionRequest, jsonToTemplateConfig } from '@/lib/types'
...
export async function getAllDoctorsWithStats(): Promise<ApiResponse<DoctorWithStats[]>> {
  try {
    ...
    // Get all doctors with their consultation counts
    // The 'consultations(count)' part will add a 'consultations' array property to the doctor object
    ...
    // Get this month's AI generations for each doctor
    ...
    const doctorsWithStats: DoctorWithStats[] = await Promise.all(
      doctors.map(async (doctor) => {
        // *** CRITICAL CHANGE: Destructure 'consultations' out of the `doctor` object ***
        // 'restOfDoctorData' will now contain all properties EXCEPT 'consultations'
        ...
        const { count: thisMonthGenerations } = await supabase
          .from('usage_logs')
          .select('*', { count: 'exact', head: true })
          .eq('doctor_id', restOfDoctorData.id) // Use restOfDoctorData.id
          .eq('action_type', 'ai_generation')
        ...
        return {
          // Spread the 'restOfDoctorData' which now aligns with DoctorWithStats by omitting 'consultations'
          ...restOfDoctorData,
          // Re-map nullable DB fields to be optional (undefined) as your code uses ?? undefined
          ...
          // Assign the derived total_consultations from the destructured 'consultations'
...
export async function performAdminAction(request: AdminActionRequest): Promise<ApiResponse<boolean>> {
  try {
    ...
    switch (request.action) {
      ...
      case 'reject':
        // For now, we'll just delete the doctor record
        // In production, you might want to keep a record and mark as rejected
